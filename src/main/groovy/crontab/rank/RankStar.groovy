...
class RankStar {
    ...
    /**
     ...
     user_id:row._id as Integer,num:finance['bean_count_total'] as Long,sj:new Date()))}coll.remove(new BasicDBObject("cat",cat))
     if(list.size() > 0)
     coll.insert(list)}*/
    ...
    static void staticTotal() {
        ...
        def res = roomStarDB.aggregate(
                $$('$project', [_id: '$star_id', earned: '$num']),
                $$('$group', [_id: '$_id', num: [$sum: '$earned']]),
                $$('$sort', [num: -1])
        ).results()
        int i = 0
        ...
        res.each {
            row ->
                ...
                if (user_id) {
                    ...
                    rank_total_list.add($$(_id: "${cat}_${user_id}".toString(), cat: cat, user_id: user_id, num: row.num, rank: ++i, sj: new Date()))
                }
        }
    ...
    static staticFamilyTotal() {
        try {
            def timeLimit = new BasicDBObject(timestamp: [$gte: System.currentTimeMillis() - 2 * DAY_MILLON], pic_url: [$exists: true], type: 2)
            // 最近一天开播过家族直播间的
